...
export class AssistLine extends Emiter<Topic> {
    ...
    if (receiver) {
      for (let child of receiver.getChildren()) {
        ...
        lines.push({
          ...
          distance : Math.abs(nodeRect.bottom() - childRect.bottom())
        })
        lines.push({
          dir: 1,
          type: 1, 
          pos:childRect.left,
          distance : Math.abs(nodeRect.left- childRect.left)
        })
        lines.push({
          dir: 1,
          type: 1, 
          pos:childRect.right(),
          distance : Math.abs(nodeRect.right()- childRect.right())
        })
        lines.push({
          dir: 0,
          type: 0, 
          pos:childRect.centerY(),
          distance : Math.abs(nodeRect.centerY()- childRect.centerY())
        })
        ...
        lines.push({
          ...
          distance : Math.abs(nodeRect.centerX()- childRect.centerX())
        })
  ...
  move(receiver: Node) {
    if(this.node) {
      this.emit(Topic.AssistLinesChanged, this.calculateLines(this.node, receiver))
    }
  }
